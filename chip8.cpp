...

{
    ...
    {
        //copying the fonts onto memory starting at 0x50
...
void Chip8::Cycle()
{
    ...
    //decrements delaytimer if it exists
    if(delayTimer > 0)
    {
        --delayTimer;
    }

    //decrements soundTimer if it exists
    if(soundTimer > 0)
    {
        --soundTimer;
    }
...
void Chip8::OP_8xy6()
{
	...
	// Save least significant bit in 0xf
...
void Chip8::OP_Dxyn()
{
	...
	for (unsigned int row = 0; row < height; ++row)
	{
		...
		for (unsigned int col = 0; col < 8; ++col)
		{
			...
			uint32_t* screenPixel = &video[(yPos + row) * DISPLAY_WIDTH + (xPos + col)];
...
//copies timer over from the Vx register
void Chip8::OP_Fx15()
{
	uint8_t Vx = (opcode & 0x0F00u) >> 8u;
